    ...
    wx.onSocketMessage(function(res) {
      ...
      if (res && res.data == '连接成功') {
        ...
        for (let d = 0; d < dat.data.length; d++) {
					if(d==0){
						if(dat.data[d].status == 1){dat.data[d].status = 4;}
        ...
        if (_this.ArrayValueChange(_this.data.meeting.party, dat.party, 'id').changed) {
          _this.setData({
            'meeting.party': dat.party
				...
				// 设置顶部申请发言列表
				if (_this.ArrayValueChange(_this.data.meeting.speakList, dat.apply, 'id').changed) {
				  _this.setData({
